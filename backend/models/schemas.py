from sqlalchemy import Column, Integer, String, DateTime, Text, ForeignKey  # type: ignore[import]
...
class MouCompany(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    company_code = Column(String, unique=True, nullable=True)  # e.g. "MOU-0001"
    name = Column(String, nullable=False)
    ...
    pan = Column(String, nullable=True)
    trainer_contact = Column(String, nullable=True)  # trainer Aadhaar/phone captured on the MOU
    created_at = Column(DateTime(timezone=True), server_default=func.now())